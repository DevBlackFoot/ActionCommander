...
GoblinArcher::GoblinArcher(SanyoniEngine::GameObject* _gameObject, tstring _typeName) :
	...
	m_LastEndAttackTimeMillis(0)
{
	...
	GetGameObject()->AddComponent<GoblinArcherRigidBodyController>();
	...
	GetGameObject()->AddComponent<GoblinArcherAnimation>();
	GetGameObject()->AddComponent<GoblinArcherAnimationEventHandler>();
	...
	_detectTriggerCollider->AddComponent<DetectTriggerCollider>()->SetColliderSize(600, 50);

	// 공격용 트리거 설치
	GameObject* _attackTriggerCollider = new GameObject(GetGameObject()->GetScene());

	GetGameObject()->AddChild(_attackTriggerCollider);
	//_attackTriggerCollider->GetComponent<Transform>()->SetLocalPosition({ 400.f * m_LookDirection, -25.f });
	_attackTriggerCollider->AddComponent<AttackTriggerCollider>()->SetColliderSize(150, 15);
	...
	GetGameObject()->GetComponent<Health>()->SetOnHealthChanged([&](int _amount)
		{
			...
			// 속력자체를 줄여야..
			GetGameObject()->GetComponent<GoblinArcherRigidBodyController>()->SetChaseSpeed(0);
...
void GoblinArcher::OnStart()
{
	ComponentBase::OnStart();

	// 애니메이션 클립을 가져옵니다.
	m_IdleAnimClip = Resources::Instance.LoadAnimationClipFromJsonFile(TEXT("Assets/Animation/Enemy/GoblinArcher_Idle.json"));
	m_AttackAnimClip = Resources::Instance.LoadAnimationClipFromJsonFile(TEXT("Assets/Animation/Enemy/GoblinArcher_Attack.json"));

	m_DeadAnimClip = Resources::Instance.LoadAnimationClipFromJsonFile(TEXT("Assets/Animation/Enemy/GoblinArcher_Dead.json"));
	m_HitAnimClip = Resources::Instance.LoadAnimationClipFromJsonFile(TEXT("Assets/Animation/Enemy/GoblinArcher_Hit.json"));

	m_WalkAnimClip = Resources::Instance.LoadAnimationClipFromJsonFile(TEXT("Assets/Animation/Enemy/GoblinArcher_Walk.json"));
	...
	Resources::Instance.LoadSpriteSheetFromJsonFile(TEXT("Assets/SpriteSheet/Enemy/GoblinArcher_Idle.json"));
	Resources::Instance.LoadSpriteSheetFromJsonFile(TEXT("Assets/SpriteSheet/Enemy/GoblinArcher_Attack.json"));
	Resources::Instance.LoadSpriteSheetFromJsonFile(TEXT("Assets/SpriteSheet/Enemy/GoblinArcher_Dead.json"));
	Resources::Instance.LoadSpriteSheetFromJsonFile(TEXT("Assets/SpriteSheet/Enemy/GoblinArcher_Walk.json"));
...
void GoblinArcher::Tick()
{
	ComponentBase::Tick();

	const auto _controller = m_GameObject->GetComponent<GoblinArcherRigidBodyController>();
	if (_controller == nullptr)
		return;

	const auto _goblinArcherAnimation = m_GameObject->GetComponent<GoblinArcherAnimation>();
	if (_goblinArcherAnimation == nullptr)
		return;
	...
	// 아처는 너무 가까이 다가오면 안 때리도록
	_attackTriggerCollider->GetGameObject()->GetComponent<Transform>()->SetLocalPosition({ 200.f * m_LookDirection, -35.f });
...
void GoblinArcher::TakeDamage(int _damage, const Vector2& _push)
{
}

std::vector<Damagable*> GoblinArcher::TryAttack(int _damage, const Collision::AABBBounds& _bounds,
	const Vector2& _push)
{
...
void GoblinArcher::SetLookDirection(int _lookDirection)
{
